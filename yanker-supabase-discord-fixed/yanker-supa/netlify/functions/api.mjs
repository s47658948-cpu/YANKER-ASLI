      ...
      if (decision === "approve") {
        ...
        if (existing?.length) {
          ...
          const m = await db("members", {
            ...
            body: JSON.stringify({ id: crypto.randomUUID(), name: request.name, username: request.username, password_hash: request.password_hash || "", discord: request.discord, rank: request.rank || "Recruit", status: "online", joined_at: Date.now(), source_request_id: request.id })
          });
