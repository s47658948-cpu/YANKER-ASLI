...
const dataStore = getStore("yanker-data");

async function getData(key) {
  const value = await dataStore.get(key, { type: "json", consistency: "strong" });
  return Array.isArray(value) ? value : [];
}

async function setData(key, value) {
  await dataStore.setJSON(key, Array.isArray(value) ? value : []);
}
...
export async function handler(event) {
  ...
  try {
    ...
    // Return only the logged-in user's own request history and membership.
    // The passwordHash is used as a credential because the front-end account
    // currently lives in localStorage rather than a server-side user table.
    if (event.httpMethod === "POST" && action === "my-status") {
      const username = String(body.username || "").trim();
      const passwordHash = String(body.passwordHash || "").trim();

      if (!username || !passwordHash) {
        return reply(400, { ok: false, error: "اطلاعات حساب کامل نیست." });
      }
      ...
      const ownRequests = requests
        .filter(r =>
          ...
          String(r.passwordHash || "") === passwordHash
        )
        .sort((a, b) => b.createdAt - a.createdAt);

      const member = members.find(m =>
        String(m.username).toLowerCase() === username.toLowerCase() &&
        String(m.passwordHash || "") === passwordHash
      );

      if (!ownRequests.length && !member) {
        return reply(401, { ok: false, error: "حساب کاربری معتبر نیست." });
      }
