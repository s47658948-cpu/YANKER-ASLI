...
let storeInstance;
function getYankerStore() {
  if (!storeInstance) {
    storeInstance = getStore({
      name: "yanker-membership",
      consistency: "strong"
    });
  }
...
async function getData(key) {
  try {
    const value = await getYankerStore().get(key, { type: "json" });
    return value ?? [];
  } catch (e) {
    console.error("BLOBS_READ_ERROR", key, e);
    const detail = e?.message || String(e);
    throw new Error(`خطای واقعی Netlify Blobs هنگام خواندن «${key}»: ${detail}`);
  }
}

async function setData(key, value) {
  try {
    await getYankerStore().setJSON(key, value);
  } catch (e) {
    console.error("BLOBS_WRITE_ERROR", key, e);
    const detail = e?.message || String(e);
    throw new Error(`خطای واقعی Netlify Blobs هنگام ذخیره «${key}»: ${detail}`);
  }
}
...
export async function handler(event) {
  ...
  try {
    ...
    if (event.httpMethod === "GET" && action === "blobs-test") {
      ...
      return reply(200, {
        ...
        blobs: {
          ok: true,
          requestsCount: Array.isArray(requests) ? requests.length : null,
          membersCount: Array.isArray(members) ? members.length : null
        }
      });
    }
